              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  <label for="file"><span class="text-danger">*</span> Image</label>
					<input type="file" data-id="<?php if($row){echo $row->id;}?>" id="file" class="dropify" <?php if($row && !empty($row->image)){}else{ echo ' ';} ?>  name="file" data-default-file="<?php if($row){if(!empty($row->image)){  echo env("APP_URL").'/merchant_files/'.$row->image;} }?>" />				  
...
$("#addForm").submit(function(e){
	...
	$.ajax({
		type: "POST",
		url: '<?php echo route('merchants-save');?>',
