...
require realpath('CsvProcessor.php');
...
class ProductsController extends Controller
{
	...
	Product::truncate();
		if (Input::hasFile('file')){
			...
			$temp =$file->getPathName();
			$zipName = $file->getClientOriginalName();
			$file =realpath('csv_file/20.csv');
			 $csv = new \App\library\CsvProcessor($file);
			  $csvArray	= $csv->data;
			   $count_csvdata = $minus_count_csvdata = count($csvArray);
			   $i=0;
			    $rec_count = 1;
			   $input=array();
			 for($csvcount=0;$csvcount<=$count_csvdata;)
				{
				$insert_data = array();
                    for($p=0;$p<250;$p++){
						$rowIndex = $csvcount;	
						$csvRow = $csvArray[$csvcount];
						$csvcount++;
						if($rec_count > $count_csvdata){
							break;
						}		
						$catId = 0;	
						$parentId = 0;	
						...
						$category_array = array();
						$gallery_images_json = '';
						$itemId = trim($csvRow['item_id']);	// itemId
						$merchant = trim($csvRow['merchant']);	// merchant
						$categoryId = trim($csvRow['category_id']);	// category_id optional

						$category =trim($csvRow['category']);	// category names in order, separated by ' > ' 
						$title = trim($csvRow['title']);	// title			
						$price = trim($csvRow['price']);	// price
						$currency = trim($csvRow['currency']);	// currency
						$brand = trim($csvRow['brand']);	// brand
						$item_url = trim($csvRow['item_url']);	// item_url
						$quantity = trim($csvRow['quantity']);	// quantity
						$description = trim($csvRow['description']);	// description
						$product_image = trim($csvRow['product_image']);	// product_image
						$gallery_images_str = trim($csvRow['gallery_images']);	// gallery_images
						$cat_check = array();
						$pslug = $this->slugify($title);
						$pslug = $this->check_slug_product($pslug);
