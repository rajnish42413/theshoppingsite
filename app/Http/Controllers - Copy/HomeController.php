	...
	public function search_form(Request $request){ //multiple suggestions
		...
		if($request->isMethod('post')){
			...
			if($request->input('keyword') != ''){
				...
				if($keyword){
					$first = $first->where(function ($query) use($keyword) {
						for($s = 0; $s < count($keyword); $s++){
							$query->orWhere('categories.categoryName','like',"%$keyword[$s]%");
						}      
					});
				...
				if($keyword){
					$second = $second->where(function ($query2) use($keyword) {
						for($s = 0; $s < count($keyword); $s++){
							$query2->orWhere('brands.name','like',"%$keyword[$s]%");
						}      
					});
				}				
				...
				$results = $products->union($first)->union($second)->get(); 
				...
				if($results && $results->count() > 0){
					...
					foreach($results as $row){
						...
						array_push($keyword_array,$k_val);
					}
					...
					if($keyword_array){
						...
						foreach($keyword_array as $ka){
							...
							$output .= '<div class="col-md-6"><a class="btn-link" href="'.$url.'">'.$ka.'</a></div>';
						}
					}					
				}		
			}
