...
class HomeController extends Controller
{
	...
	public function search_data(){ //get form submit
	...
	public function search_form(Request $request){ //multiple suggestions
		...
		if($request->isMethod('get') && $request->input('search_value') !=''){
			$search_value = array();
			$search_value = $this->getParts($request->input('search_value'));
			
			$first = Brand::select(DB::raw('brands.name AS name, brands.slug AS pid , "brand" AS type, categories.slug AS custom'))->Join('products',function ($join){$join->on('products.brand_id','=','brands.id'); })->Join('categories',function ($join){$join->on('categories.categoryId','=','products.categoryId'); })->where('brands.status',1);
			
			if($search_value){
				$first = $first->where(function ($query) use($search_value) {
					for($s = 0; $s < count($search_value); $s++){
						$query->orWhere('brands.name','like',"%$search_value[$s]%");
					}      
				});
			}			
			
			$first = $first->groupBy('categories.categoryId');
			
			$categories = Category::select(DB::raw('categories.categoryName AS name, categories.slug AS pid , "category" AS type, "none" AS custom'))->where('categories.status',1);		
			
			if($search_value){
				$categories = $categories->where(function ($query2) use($search_value) {
					for($s = 0; $s < count($search_value); $s++){
						$query2->orWhere('categories.categoryName','like',"%$search_value[$s]%");
					}      
				});
			}			
			...
			if($row && $row->count() > 0){
				...
				return redirect($url.'?search='.urlencode($request->input('search_value')));
			}else{
			...
			if($request->input('search_value') != ''){		
				$search_value = $this->getParts($request->input('search_value'));
				//echo '<pre>'; print_r($search_value); die;
				$first = Category::select(DB::raw('categories.categoryName AS name, categories.slug AS pid , "category" AS type, "none" AS custom'))->where('categories.status',1);
				
		
				if($search_value){
					$first = $first->where(function ($query) use($search_value) {
						for($s = 0; $s < count($search_value); $s++){
							$query->orWhere('categories.categoryName','like',"%$search_value[$s]%");
						}      
					});
				}				
				
				$second = Brand::select(DB::raw('brands.name AS name, brands.slug AS pid , "brand" AS type, categories.slug AS custom'))->Join('products',function ($join){$join->on('products.brand_id','=','brands.id'); })->Join('categories',function ($join){$join->on('categories.categoryId','=','products.categoryId'); })->where('brands.status',1);
				

				if($search_value){
					$second = $second->where(function ($query2) use($search_value) {
						for($s = 0; $s < count($search_value); $s++){
							$query2->orWhere('brands.name','like',"%$search_value[$s]%");
						}      
					});
				}				
				
				
				$second = $second->groupBy('categories.categoryId');
				...
				if($search_value){
					$products = $products->where(function ($query3) use($search_value) {
						for($s = 0; $s < count($search_value); $s++){
							$query3->orWhere('products.title','like',"%$search_value[$s]%");
						}      
					});
				}				
